...
export default function HomePage() {
  return (
    <div className="h-screen w-full bg-[#7e22ce] relative overflow-hidden font-sans flex flex-col">
      ...
      <div className="absolute inset-0 bg-linear-to-br from-[#8b5cf6] via-[#7e22ce] to-[#be185d] -z-10" />
      <div className="absolute top-[-10%] right-[-10%] w-125 h-125 bg-white/10 rounded-full blur-[100px] -z-10" />
      ...
      <main className="flex-1 flex flex-col md:flex-row items-center justify-between px-8 md:px-20 pb-10">
        ...
        <div className="relative mt-8 md:mt-0 flex justify-center items-center h-full max-h-125">
          <div className="w-70 h-105 md:w-[320px] md:h-120 bg-white rounded-[2.5rem] p-3 shadow-2xl transition-transform duration-500 hover:rotate-1">
            <div className="w-full h-full bg-[#1e1b4b] rounded-4xl overflow-hidden p-6 flex flex-col">
                ...
                <div className="mt-auto pt-4 border-t border-white/5 flex items-center gap-2">
                    ...
                    <div className="w-7 h-7 bg-pink-600 rounded-full flex items-center justify-center">
                        <div className="w-2 h-2 bg-white rotate-45 -ml-px" />
                    </div>
                </div>
            </div>
          </div>
          ...
          <div className="absolute -top-4 -right-4 flex gap-1 opacity-40">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="w-0 h-0 border-l-4 border-l-transparent border-r-4 border-r-transparent border-b-8 border-b-white" />
            ))}
          </div>
        </div>
